...
function useReview(ctx: Ctx) {
  ...
  const S = useMemo<any>(() => ({ headline: null, market: "", market_slug: "", week_start: "", channel: null, queue: [] as Q[], byId: {} as any, selected: null as string | null, setRows: {} as any, setRowsList: [] as any[], receipts: {} as any, weekly: {} as any, work: {} as any, sugg: {} as any, editing: false, confirmDel: false, compose: "", adding: false, pickerQuery: "", addingGranola: false, processing: false, processSummary: "", workTab: "review", memoryOpen: false, memory: null }), []);
  ...
  const loadQueue = () => {
    ...
    S.channel = (h.notes_channels || (ctx.getBaseHeadline?.() || {}).notes_channels || {})[S.market_slug] || null;
    Promise.all([
  ...
  const loadCe = (id: string) => Promise.all([
    api.weeklyCommentary({ market_slug: S.market_slug, ce_id: id }, "", 8).catch(() => ({ weekly: [] })),
    api.granolaSuggestions({ market_slug: S.market_slug, ce_id: id, week: S.week_start } as any).catch(() => ({ suggestions: [] })),
  ]).then(([w, s]: any) => { S.weekly[id] = (w.weekly || []).find((r: any) => String(r.week_start) === String(S.week_start)) || null; S.sugg[id] = s.suggestions || []; rerender(); });
  ...
  return {
    ...
    attachGranola: (url: string) => { if (!/^https:\/\/([a-z0-9-]+\.)*granola\.ai\//i.test(url)) { toast("Paste a valid Granola link"); return; } api.attachGranolaMeeting({ market_slug: S.market_slug, ce_id: S.selected, ce_name: (S.byId[S.selected] || {}).ce_name, week_start: S.week_start, source_url: url, submitted_by: author() || "BGM" }).then(() => { S.addingGranola = false; toast("Meeting attached"); loadCe(S.selected); }).catch((e: any) => toast(e?.message || "Could not attach")); },
    saveWork: (item: any, msg: string) => api.saveWork(item).then(() => { toast(msg); reloadWork(); }),
...
function CEView({ r, q }: { r: R; q: Q }) {
  ...
  const setTreatment = (v: string) => api.saveReviewSetItem({ market_slug: S.market_slug, week_start: S.week_start, ce_id: q.ce_id, ce_name: q.ce_name, treatment: v, reason: q.reason, source: q.source }).then((res: any) => { S.setRows[q.ce_id] = res.review_set_item || { treatment: v }; r.rerender(); });
  const saveNote = () => { if (!note.trim()) { toast("Write an observation first"); return; } api.saveWeeklyNote({ market_slug: S.market_slug, ce_id: q.ce_id, ce_name: q.ce_name, week_start: S.week_start, bgm_note: note.trim(), bgm_author: author() || "BGM" }).then((res: any) => { S.weekly[q.ce_id] = res.weekly; S.editing = false; toast("Note saved"); r.rerender(); }); };
  const delNote = () => api.deleteWeeklyNote({ market_slug: S.market_slug, ce_id: q.ce_id, week_start: S.week_start }, author() || "BGM").then((res: any) => { S.weekly[q.ce_id] = res.weekly || null; S.confirmDel = false; toast("Note deleted"); r.rerender(); });
  const toggle = (w: any) => r.saveWork({ ...w, status: CLOSED.indexOf(w.status) >= 0 ? (w.kind === "check" ? "scheduled" : "needs_action") : "complete" }, "Updated");
  const finish = () => api.finishReview({ market_slug: S.market_slug, ce_id: q.ce_id, ce_name: q.ce_name, week_start: S.week_start, treatment: t, reviewer: author() || "BGM", open_work_count: String(r.openWork(q.ce_id).length), summary: hasNote ? weekly.bgm_note : "" }).then((res: any) => { S.receipts[q.ce_id] = res.receipt || {}; toast("CE review finished"); r.rerender(); });
...
function GranolaAdd({ r }: { r: R }) { const [url, setUrl] = useState(""); return <div style={{ margin: "10px 0" }}><div style={{ display: "flex", gap: 8 }}><input value={url} onChange={(e) => setUrl(e.target.value)} placeholder="https://notes.granola.ai/t/… — paste a meeting link" style={{ ...inp, flex: 1 }} /><Button size="small" variant="primary" btnType="primary" primaryText="Add meeting" onClick={() => r.attachGranola(url.trim())} /><Button size="small" variant="secondary" btnType="black" primaryText="Cancel" onClick={() => { r.S.addingGranola = false; r.rerender(); }} /></div><div style={{ marginTop: 6, color: g500, fontSize: 12 }}>Use only when a meeting wasn’t matched automatically. It’s queued for extraction.</div></div>; }
