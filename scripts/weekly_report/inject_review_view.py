...
def build_boot(deploy: Path, native: bool = False):
    if native:
        ...
        boot = (
            ...
            "    getHeadline:currentHeadline, getBaseHeadline:baseHeadline,\n"
...
def resolve_targets(args) -> list[Path]:
    ...
    if args.all:
        return sorted(p for p in deploy.glob("weekly-report-*.html") if not re.search(r"-\d{4}-\d{2}-\d{2}\.html$", p.name))
    if args.market:
...
def main(argv=None):
    ...
    ap.add_argument("--all", action="store_true", help="all current-week aliases in the deploy dir")
