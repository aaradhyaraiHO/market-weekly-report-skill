...
async function suggestionsFor(meeting, match) {
  ...
  return (Array.isArray(result.suggestions) ? result.suggestions : []).slice(0, 20).map((item, index) => ({
    market_slug: match.market_slug, week_start: match.week_start, ce_id: match.ce_id, ce_name: match.ce_name,
    source_type: "granola", source_author: meeting.author, source_ref: `${sourceRef}:${item.kind}:${index + 1}`,
...
export default async function handler(req, res) {
  ...
  if (!sameSecret(req.headers["x-review-secret"], process.env.REVIEW_MODE_AI_WEBHOOK_SECRET))
  ...
  try {
    const notes = await listNotes(clean(body.updated_after, 20) || week);
    const items = [];
